...
然后访问 `http://127.0.0.1:7880/api/health` 和
`http://127.0.0.1:7880`。包内 `outputs/` 已包含一组真实图片结果和一组
真实视频结果，启动后可直接从任务历史重新打开；无需现场训练模型。

若课程机的 `yolo` 环境缺少 FFmpeg，请先安装 FFmpeg 或改用下方 Docker
正式验收路径。
...
- 最终证据、Bug、贡献与演示材料分别见
  ...
  [贡献核验](docs/CONTRIBUTIONS.md)、[任务书提交核验](docs/SUBMISSION_AUDIT.md)
  和 [8 分钟演示稿](docs/DEMO_SCRIPT.md)。
